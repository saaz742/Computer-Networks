...
struct msg message;
{
  if (!A_accept_data)
  {
    printf("A_output: Data not accepted\n");
    return -1;
  }
  struct pkt packet;
...
void B_input(packet) struct pkt packet;
{
  ...
  struct pkt ack_pack;
  strcpy(message.data, packet.payload);
  ack_pack.checksum = cal_checksum(ack_pack);
  ack_pack.acknum = packet.seqnum;
  B_seq_num = !B_seq_num;
  last_pkt = ack_pack;
  tolayer5(B, message);
  tolayer3(B, ack_pack);
